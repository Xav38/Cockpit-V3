...
const AttributionsTable = ({ projectData = [] }: { projectData?: ProjectDataType[] }) => {
  ...
  const [etapeFilter, setEtapeFilter] = useState<string[]>([])
  ...
  const filteredData = useMemo(() => {
    ...
    // Filtrage par étape
    if (etapeFilter.length > 0) {
      filtered = filtered.filter(project => etapeFilter.includes(project.etape))
    }
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        ...
        <Box sx={{ p: 2, display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <Autocomplete
            ...
            value={options?.statuts?.filter(s => statusFilter.includes(s.value)) || []}
            onChange={(_, newValue) => {
            ...
            renderOption={(props, option) => (
              <li {...props}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <i 
                    className={classnames('ri-circle-fill text-xs', {
                      'text-info': option.value === 'nouveau',
                      'text-primary': option.value === 'en_cours', 
                      'text-success': option.value === 'termine',
                      'text-error': option.value === 'annule',
                      'text-warning': option.value === 'bloque'
                    })} 
                  />
          ...
          <Autocomplete
            multiple
            size="small"
            options={options?.etapes || []}
            getOptionLabel={(option) => option.label}
            value={options?.etapes?.filter(e => etapeFilter.includes(e.value)) || []}
            onChange={(_, newValue) => {
              setEtapeFilter(newValue.map(option => option.value))
            }}
            renderInput={(params) => (
              <TextField
                {...params}
